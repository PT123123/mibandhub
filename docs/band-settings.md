...
- 对应官方 App「心率检测 → 检测模式」里的**自动心率检测**档（GB 中文界面上
  ...
  0 就是官方那个「关闭」；
- 我们默认**关**（应用侧 `SET_AUTO_HR` 缺省 false，连接后下发 `14 00`）；
  打开时按 `SET_AUTO_HR_INTERVAL` 下发的档位探测，**默认 30 分钟** ——
  官方标称 15 天续航的测试条件正是「以 30 分钟为频率的自动心率检测」，
  官方 FAQ 给省电建议时也推荐这一档；界面上的「检测频率」可选
  1 / 5 / 10 / 30 分钟（`BandPrefs.AUTO_HR_INTERVALS`，1 分钟接近连续测量，最费电）；
