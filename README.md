...
| 东西 | 位置 |
|---|---|
| 密钥 + 口令（工作副本） | 仓库外 `C:\Users\ted\keystores\debug.keystore`（alias `androiddebugkey`，口令 `android`）+ 仓库根 `keystore.properties`，均被 `.gitignore` 覆盖 |
| 备份（唯一无法补救的资产） | `C:\Users\ted\keystores\`（debug.keystore 本体，仓库外） |
...
`just release` 的自检是硬门槛：**只认证书 SHA256 白名单里的正式 key**（现在是 `C:\Users\ted\keystores\debug.keystore`）、校验包名与
